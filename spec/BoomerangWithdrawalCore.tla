...
(* - now includes signing export and WT broadcast in abstract form         *)
(*                                                                         *)
...
WTBundleFreshAtWitness(m, witnessHeight) ==
    ...
    IN /\ m # NoMessage
       /\ WTBundleWellFormed(m)
       /\ WTApprovalFreshAtPeer(wtAppr, witnessHeight)
       /\ ApprovalFreshRelative(initAppr, wtAppr.height)
            ...
            if peer_state[self] = "DiggingGame" then
                either
                    await reached_collection_delivered[self]
                       ...
                       /\ \A j \in Peers :
                            /\ reached_pings_collection[j] # NoMessage
                            /\ ValidSig(reached_pings_collection[j], j)
                            /\ ReachedPingMatchesSessionTx(
                                reached_pings_collection[j],
                                session_id,
                                tx_id);
                    peer_accepted_sar_ack[self] := sar_ack_i[self];
                    peer_accepted_sar_ack_expected[self] := saved_expected_sar_ack[self];
...
VARIABLES mystery_i, used_sessions, completed_withdrawals, session_id, tx_id, 
          ...
          initiator_commit_acked, peer_tx_commit_collection, 
          commit_collection_delivered, saved_expected_sar_ack, 
          sar_pending_ack_request, sar_ack_i, sar_seen_placeholder_ids_i, 
          sar_escalated_i, wt_round_pings, wt_last_accepted_ping, 
          peer_reviewed_ping_history_i, pong_i, reached_collection_delivered, 
          reached_pings_collection, counter_i, ping_seq_num_i, 
          reached_mystery_flag_i, reached_boomlets_collection_i, 
          signed_psbt_outbox, wt_signed_psbt_collection, broadcast_record, 
          wt_accepted_approval, wt_accepted_approval_height, 
          peer_accepted_wt_bundle, peer_accepted_wt_bundle_height, 
          wt_accepted_commit, wt_accepted_commit_height, wt_accepted_ping, 
...
CollectCommitments == /\ pc[WT_ID] = "CollectCommitments"
                      /\ IF wt_state = "CollectingCommitments"
                            THEN /\ \/ /\ \E i \in {p \in OtherPeers(INITIATOR) :
                                       ...
                                       /\ UNCHANGED <<wt_state, commit_outbox, initiator_commit_acked, peer_tx_commit_collection, commit_collection_delivered, sar_pending_ack_request, wt_accepted_commit, wt_accepted_commit_height>>
                                    \/ /\ \E i \in {p \in Peers :
                                                        /\ commit_outbox[p] # NoMessage
                                                        /\ peer_tx_commit_collection[p] = NoMessage
                                                        /\ sar_pending_ack_request[p] = NoMessage
                                            ...
                                            /\ sar_pending_ack_request' = [sar_pending_ack_request EXCEPT ![i] =                           ExpectedAck(
                                                                                                                 ...
                                                                                                                 0,
                                                                                                                 CommitPlaceholder(commit_outbox[i]))]
                                       /\ UNCHANGED <<wt_state, approval_bundle_outbox, approvals_bundle_accepted, commit_outbox, initiator_commit_acked, peer_tx_commit_collection, commit_collection_delivered>>
                                    \/ /\ \E i \in {p \in Peers :
                                                        /\ commit_outbox[p] # NoMessage
                                                        /\ AckMatchesExpected(
                                                            sar_ack_i[p],
                                    ...
                                    \/ /\    AllCommitsPresent(peer_tx_commit_collection)
                                       ...
                                       /\ commit_collection_delivered' = [i \in Peers |-> TRUE]
                                       /\ wt_state' = "CollectingPings"
                                       /\ UNCHANGED <<approval_bundle_outbox, approvals_bundle_accepted, commit_outbox, initiator_commit_acked, peer_tx_commit_collection, sar_pending_ack_request, wt_accepted_commit, wt_accepted_commit_height>>
...
AckLoop == /\ pc[SAR_ID] = "AckLoop"
           ...
           /\ UNCHANGED << mystery_i, used_sessions, completed_withdrawals, 
                           ...
                           wt_accepted_approval, wt_accepted_approval_height, 
                           peer_accepted_wt_bundle, 
                           peer_accepted_wt_bundle_height, wt_accepted_commit, 
