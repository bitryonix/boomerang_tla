# Boomerang Assumption Register And Derived Design Gaps

## Purpose

This document turns the repository's stated, implied, and WIP trust assumptions into a single assumption register, then derives a design-gaps table from those assumptions only.

The goal is not to restate the protocol. The goal is to answer:

1. What must already be true for the current design to work as intended?
...
- the protocol cannot work without the assumption;
...
### A. Baseline, Scope, And Canonicality

| ID | Assumption | Sources | If False | Status |
| --- | --- | --- | --- | --- |
| AR-01 | The current Markdown docs and sequence diagrams are the canonical description of Boomerang behavior. | `R`, `DD`, `SU`, `WD`, `SP`, `WI`, `WN` | Different components or future implementations may satisfy different protocol semantics. | Explicit |
| AR-02 | WIP security artifacts are informative but not yet authoritative. | `SM`, `WIP-SI`, `WIP-WI`, `WIP-WF`, `WIP-LD` | The register would overstate maturity and understate unknowns. | Explicit |
...
| AR-05 | Research-stage operational guidance can temporarily stand in for missing protocol mechanisms. | `R`, `DD`, `FD` | Open issues like rollover, failover, and device replacement become immediate blockers, not deferred work. | Explicit |
...
## Formal Modeling Assumptions Added By The TLA+ Spec

This section records the security assumptions that became explicit while writing the current withdrawal-focused TLA+ model in this repository. These are proof-boundary assumptions: they explain which trust and attacker-model conditions must hold for the checked TLC results to mean what we want them to mean.
...
- `FM-04`: freshness evidence is trustworthy at the modeled level.
  The model abstracts freshness to `seen_block`, `blockHeight`, and `FreshnessWindow`.
  Why this matters: it assumes those values are not attacker-forged within the modeled boundary, even though deep reorgs, split chain views, and compromised block oracles are not represented directly.
...
- `FM-12`: any future implementation-to-model trace validation must preserve the same security projection used by the current withdrawal model.
  The checked state predicates assume that implementation events could be projected faithfully onto the same session, transaction, placeholder, and readiness concepts used here.
  Why this matters: if a future implementation trace omits or distorts a security-relevant event, any conformance claim built on top of this model could overstate reality.

### Additional proof-boundary assumptions exposed by the merged withdrawal model
...
- `FM-15`: hydration and signing preserve the committed transaction intent in a way every honest peer can verify.
  The model has an explicit hydration step and requires it to equal the locked `tx_id`, but it abstracts away satisfiability checks, exact sighash/outputs/fees verification, and the independent operator-side PSBT review discussed in the design corpus.
...
- `FM-17`: liveness claims rely on explicit fairness assumptions, not only on enabled transitions.
  The repository now carries a separate `SpecWithFairness` and liveness cfg for narrow service-progress properties.
  Why this matters: claims like "fresh pending SAR acknowledgements are eventually delivered" or "loss eventually triggers fallback" do not follow from safety-only `Spec`.

- `FM-18`: freshness is modeled as an acceptance-time witness carried in per-peer / per-service witness maps.
  The model now records witness heights for accepted approvals, WT bundles, commits, pings, pongs, and accepted SAR acknowledgements.
  Why this matters: the current proof surface is stronger than a transition-local freshness guard, but the design docs still do not define canonical transcript logging for those witnesses.

- `FM-19`: the model assumes exactly one SAR per peer.
  The canonical withdrawal model now treats SAR as single per peer, with one acknowledgement stream and one replay-memory set per peer. Any old repo-owned wording suggesting multiple SARs per peer was a repo bug.
...
- `FM-22`: placeholder instances are modeled as opaque ciphertext identities with hidden owner and plaintext class.
  Every commit/ping placeholder instance is unique, one-use, and mapped internally to its owning peer and plaintext class (`padding` or `doxing_key`).
  Why this matters: this is the model-side replacement for the design's "fresh IV gives fresh ciphertext" story, but the design docs still do not define one canonical transcript representation for that placeholder lifecycle.

- `FM-23`: replay suppression is modeled at SAR as per-peer placeholder-instance memory.
  SAR accepts a pending acknowledgement only if the placeholder instance has not already been seen for that peer.
  Why this matters: this strengthens the replay story materially, but it is still abstract compared with the design's concrete `(boomlet_identity_pubkey, iv)` wording.
...
The gaps below are derived from the assumption register only. A gap is listed when one or more assumptions are:
...
| DG-10 | `AR-40`, `AR-41`, `AR-42`, `AR-43`, `AR-46`, `AR-47`, `AR-71` | Endpoint transition and replacement procedures are missing | The design requires moving hardware between hosts and anticipates replacing Phone/Niso/ST, but those procedures are ancillary only and not yet defined. | High |
...
| DG-25 | `AR-14`, `AR-18`, `AR-57`, `FM-18` | Freshness evidence is modeled but not yet protocol-canonical | The model now records freshness witnesses for accepted evidence, but the design docs still do not define how those witnesses are serialized, preserved, or audited later when chain height keeps moving. | High |
...
| DG-27 | `AR-15`, `AR-16`, `AR-19`, `AR-71`, `FM-20` | Signing/export transcript binding is still abstract at the protocol level | The model now binds hydration, export, and broadcast to a common signing ticket, but the docs still do not define the exact MuSig2 transcript, exporter acknowledgement contract, or cleanup ordering needed for a stronger proof. | High |
